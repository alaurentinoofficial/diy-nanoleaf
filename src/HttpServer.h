...
public:
    HttpServer(int port, ConfigManager *configs)
    {
        this->configs = configs;
        this->server = new ESP8266WebServer(port);
        this->onNotFound(notFoundDefault);
    }
    void on(String endpoint, HTTPMethod method, std::function<void (Request *, Response *)> handler)
    {
        this->server->on(endpoint, method, [=] {
            Request request = {server->arg("plain"), this->configs};
            Response response;

            handler(&request, &response);

            this->server->send(response.code, response.contentType, response.body);
        });
    }
    void onNotFound(std::function<void(Request *, Response *)> handler)
    {
    ...
    static void notFoundDefault(Request *req, Response *res)
    {
        res->code = 404;
